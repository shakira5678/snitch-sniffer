...
def earthquake(x, z):
    ...
    while endtime > time.time():
        ...
        if mc.getBlock(bx, by, bz) in [block.GLASS.id, block.GLASS_PANE.id]:
            mc.setBlock(bx, by, bz, -60, bz, block.AIR.id)
            continue
...
def sinkhole(x, z):
    ...
    for bx in range(-xdist, xdist+1):
        ...
        for bz in range(-zdist, zdist+1):
            ...
            for blk in blks:
                mc.setBlocks(
blk[0], mc.getHeight(blk[0], blk[1], blk[1], blk[0],-60, blk[1], block.AIR.id)
                .setBlocks(blk[0], -55, blk[1], blk[0], -60, blk[1], block.LAVA.id)
            for blk in blk
                .setBlocks((blk[0], y, blk[1], block.GRAVEL.id)
def geyser(x, z):
            y = mc.getHeight(x, z)
...
def heatwave(x, z):
            ...
            while time.time() < endtime:
                ...
                while blkid == block.AIR.id:
                    ...
                    blkd = mc.getBlockWithData(bx, by, bz).id
                blk = blkid
        ...
        block.WATER.id, block.WATER_FLOWING.id, block.WATER_STATIONARY.id]:
                    blk = block.WATER.id
                    blk = 1
...
def tsunami(x, z):
                ...
                while hm > -1:
                    ...
                    hm -= 0.2
            `   disaster = [
                tsunami, heatwave, meteor meteor_shower, geyser, earthquake, sinkhole]
